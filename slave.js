...
app.post(`/bot${TOKEN}`,(req,res)=>{
  ...
  const noiDung=tinNhan.text.trim(),guiTin=(text)=>bot.sendMessage(tinNhan.chat.id,text,{parse_mode:'Markdown'});
  if(noiDung==='/help'){guiTin('/status - Kiểm tra bot\n/slave <lệnh> - Chạy lệnh trên slave\n/master <lệnh> - Chạy lệnh trên master\n/help - Trợ giúp');return res.sendStatus(200);}
  if(noiDung==='/status'){
    Promise.all([
  ...
  if(noiDung.startsWith('/slave')){
    const lenh=noiDung.slice(6).trim();if(!lenh){guiTin('⚠️ Nhập lệnh sau /slave');return res.sendStatus(200);}
    if(!danhSachSlave.length){guiTin('⚠️ Không có slave nào online');return res.sendStatus(200);}
    guiTin(`🔄 Đang thực hiện \`${lenh}\` trên ${danhSachSlave.length} Slave...`);
    danhSachSlave.forEach(({url,tenMay,stt,port})=>guiRequest({hostname:new URL(url).hostname,path:'/exec',method:'POST',headers:{'Content-Type':'application/json'}},JSON.stringify({cmd:lenh}),duLieu=>guiTin(`💻 *Slave ${stt} ${tenMay} (Port:${port}):*\n\`\`\`\n${duLieu.trim()}\n\`\`\``)));return res.sendStatus(200);
  }
  if(noiDung.startsWith('/master')){
    const lenh=noiDung.slice(7).trim();if(!lenh){guiTin('⚠️ Nhập lệnh sau /master');return res.sendStatus(200);}
    guiTin(`🔄 Đang thực hiện \`${lenh}\` trên Master...`);
    exec(lenh,(loi,ketQua,loiChu)=>guiTin(`💻 *Master ${TEN_MAY} (Port:${CONG}):*\n\`\`\`\n${(ketQua||loiChu||loi?.message||'Không có output').trim()}\n\`\`\``));return res.sendStatus(200);
  }
  res.sendStatus(200);
});
...
app.post('/register',(req,res)=>{const{port,url,hostname,report}=req.body||{};if(!port||!url||!hostname)return res.sendStatus(400);
  ...
  chayLenh('[ -f neofetch/neofetch ] && ./neofetch/neofetch --stdout || (git clone https://github.com/dylanaraps/neofetch && ./neofetch/neofetch --stdout)',ketQua=>bot.sendMessage(ID_NHOM,`📩 *Slave ${stt} đăng ký:*\n*Tên máy:* ${hostname}\n*Port:* ${port}\n*URL:* ${url}\n\n\`\`\`\n${ketQua||report||''}\n\`\`\``,{parse_mode:'Markdown'}));res.sendStatus(200);
});
